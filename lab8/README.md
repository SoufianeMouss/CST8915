...
## Demo Video
[Lab 8 Video](https://www.awesomescreenshot.com/video/46962447?key=d38400a30fb1ce63971bc9514dca6961)

## Task 2 solution
1. **Replacing MongoDB**
Azure Cosmos DB is a fully managed NoSQL database service.
With the MongoDB API, it behaves like MongoDB and supports most MongoDB drivers and commands.
This is the most common replacement for MongoDB in Azure environments, as it's fully managed with high availability, no need to configure replicas or persistent volumes and most importantly it handles elastic scaling automatically.

2. **Replacing RabbitMQ**
Azure Service Bus, a fully managed enterprise message broker for asynchronous messaging, it provides reliable queues and topics without requiring you to manage any servers or clustering. It ensures durable message storage, automatic scaling, and high availability, making it a strong replacement for self-hosted RabbitMQ in cloud-native architectures.
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: order-service
          image: soufianealgonquin/order-service-l8:latest # Custom image for order service
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: makeline-service
          image: soufianealgonquin/makeline-service-l8:latest # Custom Makeline service image
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: product-service
          image: soufianealgonquin/product-service-l8:latest # Custom product service image
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-front
          image: soufianealgonquin/product-service-l8:latest # Front-end store application
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-admin
          image: soufianealgonquin/store-admin-l8:latest # Admin interface for store
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: ai-service
          image: soufianealgonquin/ai-service-l8:latest
          ports:
            - containerPort: 5001
          env:
      ...
      containers:
        - name: mongodb
          ...
          livenessProbe: # Probe to check MongoDB health
            ...
            initialDelaySeconds: 5 # Wait time before starting probe
            periodSeconds: 5 # Interval between probes
          volumeMounts:   # Mount volume for MongoDB data
            - name: mongodb-data 
              mountPath: /data/db
  volumeClaimTemplates:
    - metadata:
      ...
      spec:
        ...
        resources:
          requests:
            storage: 5Gi # Storage size for each MongoDB pod
---
# Service to expose MongoDB
apiVersion: v1
kind: Service
        ...
        - name: rabbitmq
          ...
          volumeMounts: # Mount configuration for RabbitMQ plugins
            - name: rabbitmq-enabled-plugins
              mountPath: /etc/rabbitmq/enabled_plugins
              subPath: enabled_plugins
            - name: rabbitmq-data   # NEW: Persistent storage for RabbitMQ data
        ...
        - name: rabbitmq-enabled-plugins
          configMap:
            ...
            items:
              - key: rabbitmq_enabled_plugins
                path: enabled_plugins
  volumeClaimTemplates: # NEW: PVC template for RabbitMQ persistent data
    - metadata:
        name: rabbitmq-data
      spec:
        accessModes:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-front
          image: soufianealgonquin/store-front-l8:latest # Front-end store application
          ports:
            - containerPort: 8080 # Front-end app port
              name: store-front
          env: # Environment variables for backend URLs
